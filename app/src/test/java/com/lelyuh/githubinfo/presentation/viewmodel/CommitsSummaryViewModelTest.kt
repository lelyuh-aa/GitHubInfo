...
class CommitsSummaryViewModelTest {
    ...
    private val viewModel = CommitsSummaryViewModel(gitHubInfoInteractor, RxSchedulersStub())
    ...
    fun `test success commits info`() {
        ...
        every { gitHubInfoInteractor.commitsInfo(URL) } returns Single.just(commitPresentationModel)
    ...
    fun `test error commits info`() {
        ...
        every { gitHubInfoInteractor.commitsInfo(URL) } returns Single.error(Throwable())
